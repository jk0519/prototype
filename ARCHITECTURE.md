...
The athlete collision skeleton supplies the legal striking-hand contact center. Court rendering maps the same action timers to an authored 32-frame atlas: quiet ready poses, a six-frame sprint, full spike and jump-serve sequences, and utility frames for receive, jump set, block, dive, and recovery. The contact frame is held through the physics contact window, and its visible hand is offset to the recorded contact point. Contact cannot fire twice. Run frames and footfall events track distance travelled; jump, cut, slide, and landing events drive their recorded sounds. Cosmetic motion events never reroll AI errors or increment ball-contact metrics.

Player art is original and faceless. Soft connected bodies, fitted uniforms, knee pads, hair, hands, and court shoes form clear silhouettes without eyes or mouths. North and South use separate palettes. Source studies are retained under `assets/art/animation/source` and excluded from exports; `tools/prepare_soft_animation_atlas.py` isolates the figures, removes the development background, normalizes scale and baseline, and creates the runtime atlases. The collision skeleton stays independent from input ownership, so every action remains available to any controlled role.

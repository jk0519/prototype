...
A Mac-first, 2D volleyball prototype inspired by the basic play of The Spike: six athletes, one human wing spiker, and five AI players. Built in **Godot 4.7.2**, with compact side-view athletes, deliberately exaggerated arcade motion, a reactive camera, and recorded court audio. The **0.13.0** rebuild replaces the whole-character frame atlas with articulated body-part artwork and a shared animation/contact pose.
...
The first serve is yours. Move behind the baseline with **A/D**. **Hold X** to charge a longer forward toss while carrying the ball low, adjust its height with **W/S** or the up/down arrows, then **release X** to start the throwing motion. The palm lifts for 0.22 seconds before releasing the ball into flight. Movement and the running foot cycle stay active throughout charge and throw. Press **Z** to plant and jump as the ball starts descending, then press **Z again** when it reaches your striking hand. Tossing does not jump automatically. An untouched toss loses the point; the serve must be hit in the air. After takeoff, the server can travel across the baseline and into the court. For an attack, approach the net on your side, jump as your setter's ball rises, then press Z again when the ball reaches your hand. Moving toward the net as you hit aims the spike shorter; moving away aims deeper. Hold Space before a low ball arrives to pass it to the setter.
...
- Adjustable jump serves with a low carry, continuous palm lift, hand-attached release, displayed parabola, movable charge and windup, approach, foot plant, takeoff, timed contact, follow-through, and landing compression. AI servers use the same mechanics. Grounded serve phases share one movement boundary, avoiding a position jump when charge changes to throw.
...
- Exaggerated ball gravity, severe serve and spike topspin, generous action contact areas, a lowered solid net, floor/out detection, three-touch and double-touch faults, and block touches that keep the ball live. Clean jump serves launch at extreme speed before dropping violently into the back court.
...
- Six compact, faceless athletes use original side-profile body parts: a narrow torso, profile head, distinct near/far limbs, and a modest torso turn during attacks. Continuous authored poses drive carry, throw, run, plant, jump, spike, receive, set, block, dive, and recovery. Running feet follow distance travelled even when the hands are carrying or throwing the ball. Athlete scale is 92 world units, or 98 for the middle blocker; visible screen height changes with the camera.
...
- Ball smears, directional speed lines, swing arcs, landing shock lines, short contact bursts, a brief impact wash, hit-stop, camera pullback/roll, velocity look-ahead, and fast camera tracking. Blocks accelerate the incoming spike back down at the attacker; their contact area is centered on the displayed palms.
...
`GODOT=/path/to/Godot tools/build_macos.sh` builds and zips the Mac app. The test commands above cover simulation, scene integration, contact regressions, pose continuity, and audio lifecycle. See [GAMEPLAY_REFERENCE.md](GAMEPLAY_REFERENCE.md) for the measured gameplay target, [QA.md](QA.md) for the checks and current validation limits, and [ARCHITECTURE.md](ARCHITECTURE.md) for the code layout.
